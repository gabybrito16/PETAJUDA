...
export interface Post {
  ...
  author: { name: string; initial: string }; date: string; photo?: string; species?: string;
  gender?: string; breed?: string; age?: string; whatsapp?: string; color?: string; lastSeen?: string;
}
interface PostsContextValue { posts: Post[]; loading: boolean; addPost: (post: Omit<Post, "id" | "date" | "author">) => Promise<string | null>; refreshPosts: () => Promise<void>; }
...
function toPost(row: any): Post {
  ...
  return { id: row.id, type: row.type, name: row.name || "Sem nome", neighborhood: row.neighborhood, description: row.description,
    author: { name: row.author_name || "Usuário", initial: (row.author_name || "U").charAt(0).toUpperCase() },
...
export function PostsProvider({ children }: { children: React.ReactNode }) {
  ...
  return <PostsContext.Provider value={{ posts, loading, addPost, refreshPosts }}>{children}</PostsContext.Provider>;
}
