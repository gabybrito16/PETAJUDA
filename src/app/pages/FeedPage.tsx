...
function Card({post}:{post:Post}) {
  ...
  return <article className="bg-white rounded-2xl overflow-hidden shadow-sm border border-border flex h-full flex-col"><div className="relative h-[220px] min-h-[220px] w-full shrink-0 overflow-hidden bg-slate-100">{post.photo?<><div className="absolute inset-0 scale-110 bg-cover bg-center opacity-30 blur-sm" style={{backgroundImage:`url(${post.photo})`}} aria-hidden="true"/><ImageWithFallback src={post.photo} alt={post.name} className="relative z-10 block h-full w-full object-cover object-center"/></>:<span className="text-muted-foreground text-sm">Sem foto</span>}<Badge type={post.type}/></div><div className="p-4 flex flex-col gap-2 flex-1"><h3 className="text-lg font-extrabold">{post.name}</h3><div className="flex items-center gap-1.5 text-sm text-muted-foreground"><MapPin size={14}/>{post.neighborhood}</div><p ref={descriptionRef} className={`text-sm text-primary font-medium ${expanded ? "whitespace-pre-wrap" : "line-clamp-2"}`}>{post.description}</p>{canExpand&&<button type="button" onClick={()=>setExpanded(!expanded)} className="inline-flex w-fit items-center gap-1 text-sm font-bold text-primary hover:underline" aria-expanded={expanded}>{expanded?"Ver menos":"Ver mais"}{expanded?<ChevronUp size={15}/>:<ChevronDown size={15}/>}</button>}<div className="text-xs text-muted-foreground mt-auto pt-2 border-t border-border"><b>{post.author.initial}</b> {post.author.name} · {post.date}</div></div><div className="flex flex-wrap gap-2 px-4 pb-4"><Contact post={post} label="Entrar em contato" icon={<MessageCircle size={15}/>}/>{post.type==="adoption"&&<><Contact post={post} label="Quero adotar" icon={<Heart size={15}/>}/><Contact post={post} label="Oferecer lar temporário" icon={<Home size={15}/>}/></>}</div></article>;
}
