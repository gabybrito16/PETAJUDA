...
function Card({post}:{post:Post}) {
  ...
  return <article className="bg-white rounded-2xl overflow-hidden shadow-sm border border-border flex h-full flex-col"><div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden bg-slate-100">{post.photo?<><div className="absolute inset-0 scale-[1.08] bg-cover bg-center opacity-30 blur-sm" style={{backgroundImage:`url(${post.photo})`}} aria-hidden="true"/><ImageWithFallback src={post.photo} alt={post.name} className="relative z-10 block h-full w-full object-cover object-center"/></>:<span className="flex h-full w-full items-center justify-center text-sm text-muted-foreground">Sem foto</span>}<Badge type={post.type}/></div><div className="p-4 flex flex-col gap-2 flex-1"><h3 className="text-lg font-extrabold">{post.name}</h3><div className="flex items-center gap-1.5 text-sm text-muted-foreground"><MapPin size={14}/>{post.neighborhood}</div><p ref={descriptionRef} className={`text-sm text-primary font-medium ${expanded ? "whitespace-pre-wrap" : "line-clamp-2"}`}>{post.description}</p>{canExpand&&<button type="button" onClick={()=>setExpanded(!expanded)} className="inline-flex w-fit items-center gap-1 text-sm font-bold text-primary hover:underline" aria-expanded={expanded}>{expanded?"Ver menos":"Ver mais"}{expanded?<ChevronUp size={15}/>:<ChevronDown size={15}/>}</button>}<div className="text-xs text-muted-foreground mt-auto pt-2 border-t border-border"><b>{post.author.initial}</b> {post.author.name} · {post.date}</div></div><div className="flex flex-wrap gap-2 px-4 pb-4"><Contact post={post} label="Entrar em contato" icon={<MessageCircle size={15}/>}/>{post.type==="adoption"&&<><Contact post={post} label="Quero adotar" icon={<Heart size={15}/>}/><Contact post={post} label="Oferecer lar temporário" icon={<Home size={15}/>}/></>}</div></article>;
}
export function FeedPage() {
  ...
  const postsLabel = `${visible.length} ${visible.length === 1 ? "publicação" : "publicações"}`;

  return <div className="min-h-screen bg-background flex flex-col"><Navbar variant="app" searchValue={search} onSearchChange={setSearch}/><main className="flex-1 max-w-5xl mx-auto w-full px-4 md:px-6 py-8"><div className="flex items-start justify-between mb-6 flex-wrap gap-3"><div><h1 className="text-3xl font-black">Feed</h1><p className="text-sm text-muted-foreground mt-0.5">{postsLabel}</p></div><div className="flex gap-2">{([['todos','Todos'],['adoption','Adoção'],['lost','Perdidos']] as [Filter,string][]).map(([key,label])=><button key={key} onClick={()=>setFilter(key)} className={`px-4 py-2 rounded-full text-sm font-bold ${filter===key?"bg-primary text-white":"bg-white border border-border"}`}>{label}</button>)}</div></div>{loading?<p className="text-center py-20 text-muted-foreground">Carregando publicações...</p>:visible.length===0?<p className="text-center py-20 text-muted-foreground">Ainda não há publicações.</p>:<div className="grid auto-rows-fr grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">{visible.map(post=><Card key={post.id} post={post}/>)}</div>}</main></div>;
}
