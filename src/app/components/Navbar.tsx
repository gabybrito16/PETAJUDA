...
export function Navbar({ variant = "app", searchValue = "", onSearchChange }: NavbarProps) {
  ...
  return (
    <header className="w-full bg-white border-b border-border px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
      ...
      <button onClick={async () => { await signOut(); navigate("/"); }} title="Sair" className="w-9 h-9 rounded-full flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors shrink-0">
        <LogOut size={18} />
      </button>
    </header>
  );
